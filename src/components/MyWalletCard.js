...
import { toggleWalletDetails } from '../redux/actions'; // Import your action creator

const MyWalletCard = () => {
  const dispatch = useDispatch();

  // Retrieving necessary data from the Redux store using useSelector
  const walletDetails = useSelector(state => state.walletDetails); // Replace 'state.walletDetails' with the actual path in your store

  const handleDetailsToggle = () => {
    dispatch(toggleWalletDetails()); // Dispatch action when the details button is pressed
    // Add your logic here
  };
...
const mapDispatchToProps = (dispatch) => {
  return {
    toggleWalletDetails: () => dispatch(toggleWalletDetails()), // Map your action creator to props
    // ...other action creators if needed
  };
};

export default connect(null, mapDispatchToProps)(MyWalletCard);
